<x-layouts.app title="Novo pacote" heading="Pacotes">
    <x-drawer title="Novo pacote" :close-url="route('packages.index')">
        <form method="POST" action="{{ route('packages.store') }}" class="flex min-h-0 flex-1 flex-col">
            @csrf
